...
exports.getPostPayment = async(req, res) => {
  ...
  connection.query(getIdSql, [orderId], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      const getTicketSql ="select GROUP_CONCAT(ticket_name) as ticket_names from ticket_details where id in (?)";
      const ticketidUprooted = poData.ticketid.replace(/[\[\]]/g, "").split(",").map(Number); 
      connection.query(getTicketSql,[ticketidUprooted],async(err, resultsTicket) => {
          ...
          if (resultsTicket.length > 0) {
            ...
            poData.primary_email=userData.mobile;
